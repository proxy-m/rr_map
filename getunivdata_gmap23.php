...
//$reg =3;
$_REQUEST['mode'] = (isset($_REQUEST['mode']) && !empty($_REQUEST['mode'])) ? $_REQUEST['mode'] : 'full'; // values: head, full
$_REQUEST['pos'] = (isset($_REQUEST['pos']) && !empty($_REQUEST['pos'])) ? $_REQUEST['pos'] : '0_-1'; // for LIMIT with OFFSET (only for universities)

$offset = 0; // from begin
$limit = -1; // no limit
if (false === strpos($_REQUEST['pos'], '_')) {
    $_REQUEST['pos'] = '0_-1';
}
$offset_limit = explode('_', $_REQUEST['pos']);
if (count($offset_limit) != 2) {
    $offset_limit = [];
} else {
    $offset = intval($offset_limit[0]);
    $limit = intval($offset_limit[1]);
