...
$(document).ready(function () 
{
    ...
    function addMarkers (mrks, needClickOnFirst) {
        ...
        if (!needClickOnFirst) {
            ...
            mappanel.map.getLayers().getArray().map(function (e, i) {if (i>0) mappanel.map.getLayers().getArray().splice(1) }); // rest only first layer
        } else {
		...
		if(Number($('#mapsrchvl').val().length)==0)
		{
			...
			$.ajax(
			{
	  		url: ur,
                ...
                if (window.mappanel && window.mappanel.map && window.mappanel.map.setView && window.ol && ol.View) {
                    var pos = [coord.lat, coord.lng]; /// JSON.parse('['+record.data['cord']+']');
                    pos = [pos[1], pos[0]];
                    var city = ol.proj.fromLonLat(pos);
				...
				//alert(konf[2]);
			  // Create the markers.
			  tourStops.forEach(function (position_title, i) {
			  	//alert(position.lat);
                ///var [position, title] = position_title;
                var position = position_title[0];
                var title = position_title[1];
		...
		else
		{
			...
			if(Number(unnm.length)>0 && unnmA==(unnm + ' _' + dt[$('#tphsel').val()]['id_univ']))
			{
                ...
                if (window.mappanel && window.mappanel.map && window.mappanel.map.setView && window.ol && ol.View) {
				  ...
				  const marker = new google.maps.Marker({
				    position: uluru,
				    title: '#'+getWorldRating(dt, title, null)+' - '+title, // `#${getWorldRating(dt, title, null)} - ${title}`,
				    icon:icnsrc,
				    cont:uninfo,
				    map: map,
				  });
